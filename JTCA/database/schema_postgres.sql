-- ============================================================
-- JTCA - PostgreSQL Schema
-- Run via: database/postgres_db.py → initialize_pg_schema()
-- ============================================================
...
CREATE TABLE IF NOT EXISTS tariff_rules (
    ...
    -- ETL metadata
    mongo_id        TEXT UNIQUE,          -- tracks the source MongoDB doc
    etl_processed_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE INDEX IF NOT EXISTS idx_tariff_mongo_id    ON tariff_rules(mongo_id);
